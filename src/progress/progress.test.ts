...
describe('NoopProgressReporter', () => {
  it('all methods are callable without throwing', () => {
    ...
    expect(() => {
      ...
      r.onLlmCall({ messageCount: 5, injectedTokens: 1000, systemTokens: 4000, userTokens: 5000 });
      r.onLlmResponse({
        inputTokens: 5000,
        outputTokens: 200,
        stopReason: 'end_turn',
...
describe('CliProgressReporter', () => {
  ...
  describe('--quiet flag (isTTY=true, quiet=true)', () => {
    it('suppresses all output even on a TTY', () => {
      ...
      reporter.onSessionStart('abc', false);
      reporter.onThinking();
      reporter.onLlmCall({ messageCount: 5, injectedTokens: 1000, systemTokens: 4000, userTokens: 5000 });
      reporter.onLlmResponse({
        inputTokens: 5000,
        outputTokens: 100,
        stopReason: 'end_turn',
